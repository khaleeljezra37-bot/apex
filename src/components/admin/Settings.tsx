...
export const Settings: React.FC = () => {
  ...
  const updateApiKey = (key: string, value: any) => {
    ...
    return (
      <div className="p-8 space-y-8 max-w-[1200px] mx-auto pb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 h-[600px] bg-white/5 rounded-3xl animate-pulse" />
  ...
  return (
    <div className="p-8 space-y-8 max-w-[1200px] mx-auto pb-24 text-white">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button 
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-[#00E599] text-black text-xs font-black uppercase tracking-widest hover:bg-[#00E599]/90 transition-all shadow-[0_0_20px_#00E59930] disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-[#0d0e12] border border-white/5 rounded-3xl p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  type="text"
                  value={settings.pluginVersion}
                  onChange={(e) => updateSetting("pluginVersion", e.target.value)}
                  className="w-full bg-black/40 border border-white/5 rounded-2xl py-3.5 px-5 text-sm outline-none focus:border-[#00E599]/30 transition-all"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 rounded-2xl border border-white/5 bg-white/[0.01]">
              ...
              <button 
                ...
                className={`w-12 h-6 rounded-full relative transition-all border ${settings.maintenanceMode ? 'bg-[#00E599]/20 border-[#00E599]/30' : 'bg-white/5 border-white/10'}`}
              >
                <div className={`absolute top-0.5 w-4 h-4 rounded-full transition-all ${settings.maintenanceMode ? 'right-1 bg-[#00E599]' : 'left-1 bg-white/20'}`} />
              </button>
            </div>
          </section>
          ...
          <section className="bg-[#0d0e12] border border-white/5 rounded-3xl p-8 space-y-6">
            ...
            <div className="space-y-4">
              {[
                { label: "OpenAI Secret Key", field: "openai" },
                { label: "Gemini Pro API", field: "gemini" },
                { label: "Discord Webhook URL", field: "discord" },
              ].map((key) => (
                <div key={key.field} className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-white/40 px-1">{key.label}</label>
                  <div className="relative">
                    <input
                      type={showKey === key.field ? "text" : "password"}
                      value={settings.apiKeys?.[key.field] || ""}
                      onChange={(e) => updateApiKey(key.field, e.target.value)}
                      className="w-full bg-black/40 border border-white/5 rounded-2xl py-3.5 pl-5 pr-12 text-xs font-mono outline-none focus:border-[#00E599]/30"
                    />
                    <button 
                      onClick={() => setShowKey(showKey === key.field ? null : key.field)}
                      className="absolute right-4 top-1/2 -translate-y-1/2 text-white/30 hover:text-white transition-colors"
                    >
                      {showKey === key.field ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-8">
          <section className="bg-[#0d0e12] border border-white/5 rounded-3xl p-8 space-y-6">
            ...
            <div className="space-y-4">
              {[
                { label: "Enforce 2FA", field: "enforce2fa" },
                { label: "Audit Logs", field: "auditLogs" },
                { label: "Auto-Ban Brute Force", field: "autoBan" },
              ].map((flag) => (
                <div key={flag.field} className="flex items-center justify-between">
                  ...
                  <button 
                    onClick={() => updateSetting(flag.field, !settings[flag.field])}
                    className={`w-10 h-5 rounded-full relative transition-all border ${settings[flag.field] ? 'bg-[#00E599]/20 border-[#00E599]/30' : 'bg-white/5 border-white/10'}`}
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 rounded-full transition-all ${settings[flag.field] ? 'right-0.5 bg-[#00E599]' : 'left-0.5 bg-white/20'}`} />
                  </button>
                </div>
              ))}
            </div>
