import { Search, Download, Box, LayoutTemplate, Music, Image as ImageIcon } from 'lucide-react';
import { useState } from 'react';

const mockAssets = [
  { id: 1, name: 'Low Poly Trees', type: 'model', icon: Box, creator: 'Roblox', downloads: '1.2k' },
  { id: 2, name: 'Modern UI Pack', type: 'ui', icon: LayoutTemplate, creator: 'Apex UX', downloads: '8.4k' },
  { id: 3, name: 'Footsteps Audio', type: 'audio', icon: Music, creator: 'SoundMaster', downloads: '450' },
  { id: 4, name: 'Sci-Fi Crate', type: 'model', icon: Box, creator: 'BuildBro', downloads: '3.1k' },
  { id: 5, name: 'Inventory System UI', type: 'ui', icon: LayoutTemplate, creator: 'Apex UX', downloads: '12k' },
  { id: 6, name: 'Dirt Texture 4K', type: 'image', icon: ImageIcon, creator: 'TexArts', downloads: '920' },
];
    ...
    <div className="flex-1 h-full w-full border border-white/5 bg-[#0a0a0a] rounded-2xl overflow-hidden flex flex-col shadow-2xl relative z-10 hidden sm:flex">
      <div className="p-4 border-b border-white/5 bg-[#111]">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-[13px] uppercase tracking-widest text-white/80">Asset Library</h2>
        </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto custom-scrollbar">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {mockAssets.map(asset => {
            ...
            return (
              <div key={asset.id} className="border border-white/10 bg-white/5 hover:bg-white/10 rounded-xl p-4 transition-all group flex flex-col cursor-pointer">
                ...
                <div>
                  <h3 className="font-bold text-[14px] text-white mb-1 truncate">{asset.name}</h3>
