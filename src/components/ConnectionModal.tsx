import { X, Plug, Link2, Key, Globe, Loader2, CheckCircle2 } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="relative w-full max-w-md bg-[#111] border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/[0.02]">
          <div>
            <h2 className="text-xl font-semibold text-white">Connect to Roblox</h2>
            <p className="text-sm text-white/50 mt-1">Link your live place for direct script injection.</p>
          </div>
          <button 
            onClick={onClose}
        ...
        {connected ? (
          <div className="p-10 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-xl font-medium text-white mb-2">Connected Successfully</h3>
            <p className="text-sm text-white/50">Your Roblox place is linked. AI can now inject scripts directly into your game.</p>
          </div>
        ) : (
          <div className="p-6">
            {/* Method Toggle */}
            <div className="flex bg-white/5 p-1 rounded-2xl mb-6 border border-white/5">
              <button 
                ...
                className={cn(
                  ...
                  method === "cloud" ? "bg-white/10 text-white shadow-sm border border-white/5" : "text-white/50 hover:text-white"
                )}
              >
              ...
              <button 
                ...
                className={cn(
                  "flex-1 flex items-center justify-center py-2.5 rounded-xl text-sm font-medium transition-all",
                  method === "plugin" ? "bg-white/10 text-white shadow-sm border border-white/5" : "text-white/50 hover:text-white"
                )}
              >
            ...
            <form onSubmit={handleConnect} className="space-y-4">
              {method === "cloud" ? (
                ...
                <div className="p-4 bg-white/5 border border-white/10 rounded-2xl text-center">
                  <p className="text-sm text-white/80 mb-4">Install our Roblox Studio Plugin to establish a direct local connection via HTTPService.</p>
                  <p className="font-mono text-xl tracking-widest text-white border border-white/10 bg-black/40 px-4 py-2 rounded-xl">04X-9L2</p>
                  <p className="text-xs text-white/40 mt-2">Enter this code into the plugin settings</p>
                </div>
              )}

              <button 
                type="submit"
                disabled={connecting || (method === "cloud" && (!apiKey || !universeId))}
